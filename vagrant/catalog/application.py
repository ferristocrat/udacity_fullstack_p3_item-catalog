...
from database_setup import Base, User, Activities, Subcategories
...
# Activities "make changes" page - same as main page but with edit and delete buttons
...
def activityListEdit():
	...
	activities = session.query(Activities).all()
	return render_template(
		'activities_makechanges.html', activities=activities, login_session=login_session)
...
def subCategory(activity_id):
	...
	subcategories = session.query(Subcategories).filter_by(activity_id=activity_id).all()
	return render_template(
		'activity.html', activity=activity, subcategories=subcategories, login_session=login_session)


# Items "make changes" page -  same as subCategory page but with edit/delete buttons
...
def subCategoryEdit(activity_id):
	...
	subcategories = session.query(Subcategories).filter_by(activity_id=activity_id).all()
	return render_template(
		'activity_makechanges.html', activity=activity, subcategories=subcategories, login_session=login_session)


# Edit an item
@app.route('/activities/<int:activity_id>/makechanges/<int:item_id>/edit', methods=['GET','POST'])
def itemEdit(activity_id, item_id):
	...
	if request.method == 'POST':
		...
		return render_template(
			'activity_edit.html',activity_id=activity_id, item_id=item_id, item=editedItem, login_session=login_session)


# Delete an item
@app.route('/activities/<int:activity_id>/makechanges/<int:item_id>/delete', methods=['GET','POST'])
def itemDelete(activity_id, item_id):
	...
	if request.method == 'POST':
		...
		return render_template(
			'activity_delete.html', item=itemToDelete, item_id=item_id, activity_id=activity_id, login_session=login_session)
...
def itemNew(activity_id):
	if 'username' not in login_session:
		return redirect('/login')
	if request.method == 'POST':
		newItem = Subcategories(name=request.form['name'], activity_id=activity_id, user_id=getUserID(login_session['email']))
		session.add(newItem)
		session.commit()
		flash("New activity added")
...
# Edit an activity
@app.route('/activities/makechanges/<int:activity_id>/edit', methods=['GET','POST'])
def activityEdit(activity_id):
	...
	if request.method == 'POST':
		...
		return render_template(
			'activityname_edit.html',activity_id=activity_id, activity=editedItem, login_session=login_session)


# Delete an activity
@app.route('/activities/makechanges/<int:activity_id>/delete', methods=['GET','POST'])
def activityDelete(activity_id):
	if 'username' not in login_session:
		return redirect('/login')
	itemToDelete = session.query(Activities).filter_by(id=activity_id).one()
	subItemsToDelete = session.query(Subcategories).filter_by(activity_id=activity_id).all()
	if request.method == 'POST':
		session.delete(itemToDelete)
		session.commit()
